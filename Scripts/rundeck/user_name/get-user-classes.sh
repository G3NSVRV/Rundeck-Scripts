...
curl "$curlOptions" -X "GET" -H "Content-Type: application/$apiFormat" -H "Accept: application/$apiFormat" -c "$cookie" -b "$cookie" "$rundeckServer"/api/"$apiVersion"/userclass/allocations | jq .
